        ...
        for e in pulp_nn_init.CORE_EXTENTIONS:
                            ...
                            for q in pulp_nn_init.PULPNNQuantizationMethods:
                                for sgn_in, sgn_out in product([False, True], [False, True]):
                                    for m in pulp_nn_init.MATMUL_FORMAT:
                                        ...
                                        conv=pulp_nn_factory.PULPNNConvolve1D(
                                            kernel=kernel_to_test, 
                                            layer=None
                                        )
                                        pulp_nn_init.PULPNNAPI = pulp_nn_factory.kernel(
                        ...
                        for q in pulp_nn_init.PULPNNQuantizationMethods:
                            ...
                            # TODO: add mixed PW kernels
                                if e != 'XpulpNN-mixed':
                                    ...
                                    pw=pulp_nn_factory.PULPNNConvolvePointwise(
                                        kernel=kernel_to_test, 
                                        layer=None
                                    )
                                    pulp_nn_init.PULPNNAPI = pulp_nn_factory.kernel(
                    ...
                    for sgn_in in [False, True]:
                        for lut in [False, True]:
                            ...
                            if e != 'XpulpNN-mixed':
                                ...
                                lin_nq=pulp_nn_factory.PULPNNLinearNoQuant(
                                    kernel=kernel_to_test, 
                                    layer=None
                                )
                                pulp_nn_init.PULPNNAPI = pulp_nn_factory.kernel(
                        ...
                        for q in pulp_nn_init.PULPNNQuantizationMethods:
                            ...
                            # TODO: add mixed depthwise kernels
                                    if e != 'XpulpNN-mixed':
                                        ...
                                        lin_q=pulp_nn_factory.PULPNNLinearQuant(
                                            kernel=kernel_to_test, 
                                            layer=None
                                        )
                                        pulp_nn_init.PULPNNAPI = pulp_nn_factory.kernel(
            ...
            for ip, op in product(pulp_nn_init.PULPNNDataPrecisions, pulp_nn_init.PULPNNDataPrecisions):
                for sgn_in, sgn_out in product([False, True], [False, True]):
                    ...
                    avgp=pulp_nn_factory.PULPNNAvgPoolNew(
                        kernel=kernel_to_test, 
                        layer=None
                    )
                    pulp_nn_init.PULPNNAPI = pulp_nn_factory.kernel(
                    ...
                    for sgn_in1, sgn_in2, sgn_out in product([False, True], [False, True], [False, True]):
                        for k in pulp_nn_init.PULPNNDataPrecisions:
                            kernel_to_test = pulp_nn_factory.PULPNNKernel(
                                ...
                                ext=e, 
                                mm_fmt='',
                                in_signed=[sgn_in1, sgn_in2], 
                                out_signed=sgn_out
                            )
                            add = pulp_nn_factory.PULPNNQuantAdd(
                                kernel=kernel_to_test, 
                                layer = None
                            )
                            pulp_nn_init.PULPNNAPI = pulp_nn_factory.kernel(
