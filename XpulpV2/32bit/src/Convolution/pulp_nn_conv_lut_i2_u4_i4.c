...
void pulp_nn_conv_lut_i2_u4_i4(
                        int8_t *pIn,
                        ...
                        uint8_t flag_batch_norm)
{
  ...
  for (i_out_y = start_pixel; i_out_y < stop_pixel; i_out_y++)
  {
    for(i_out_x=(section * dim_out_x_r); i_out_x<(dim_out_x_r + (section * (dim_out_x_r + flag_dim_out_x_odd))); i_out_x++)
    {
      ...
      {
        pOutBuffer = pulp_nn_matmul_lut_i8_u4_i4(
          pIm2ColBase,
          pBias,
          pOutBuffer,
          pOutBuffer + ch_out_r,
    ...
    if(pIm2Col != pIm2ColBase)
    {
      ...
      for(i = 0; i < ch_out; i++)
      {
        ...
        for(int j=0; j < col_cnt_im2col; j++)
        {
          ...
          sum = pulp_nn_look_up_i2_i32_i4(pLUT, inB, inA[0], sum);

          inB = *((v4s*) pB);

          sum = pulp_nn_look_up_i2_i32_i4(pLUT, inB, inA[1], sum);
