...
void pulp_nn_conv_lut_u2_i8_i4(
                        uint8_t *pIn,
    ...
    if(pIm2Col != pIm2ColBase)
    {
      ...
      for(i = 0; i < ch_out; i++)
      {
        ...
        for(int j=0; j < col_cnt_im2col; j++)
        {
          ...
          sum = pulp_nn_look_up_u2_i32_i4(pLUT, inB, inA[0], sum);

          inB = *((v4u*) pB);

          sum = pulp_nn_look_up_u2_i32_i4(pLUT, inB, inA[1], sum);
