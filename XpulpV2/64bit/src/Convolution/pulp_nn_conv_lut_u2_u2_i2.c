...
void pulp_nn_conv_lut_u2_u2_i2(
                        uint8_t *pIn,
    ...
    if(pIm2Col != pIm2ColBase)
    {
      ...
      for(i = 0; i < ch_out; i++)
      {
        ...
        for(int j=0; j < col_cnt_im2col; j++)
        {
          ...
          sum = pulp_nn_look_up_u2_i32_i2(pLUT, inB, inA[0], sum);

          inB = *((v4u*) pB);

          pB+=4;

          sum = pulp_nn_look_up_u2_i32_i2(pLUT, inB, inA[1], sum);

          inB = *((v4u*) pB);

          pB+=4;

          sum = pulp_nn_look_up_u2_i32_i2(pLUT, inB, inA[2], sum);

          inB = *((v4u*) pB);

          pB+=4;

          sum = pulp_nn_look_up_u2_i32_i2(pLUT, inB, inA[3], sum);
        }
